resource datadog_monitor "rabbit_high_cpu" {
  ...
  type = "metric alert"
  query = "avg(last_15m):avg:kubernetes.cpu.usage.total{kube_container_name:rabbitmq} > 500000000"
...
resource datadog_monitor "rabbit_high_mem" {
  ...
  type = "metric alert"
  query = "avg(last_5m):avg:kubernetes.memory.usage{kube_container_name:rabbitmq} > 996147000"
